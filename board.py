...
if __name__ == '__main__':
    ...
    aanzet = [BLUE, RED, RED, BLUE, None, None]

    for i,move in enumerate(MOVES):
        print >>sys.stderr, "ronde (%d) %s" % (i+1, move)
        b.move_game_notation(move, aanzet[zetteller])

        # zorg dat de juiste speler aan zet is
        if move in ['U', 'D', 'R', 'L']:
            zetteller = 0   # na een shif opnieuw beginnen
        else:
